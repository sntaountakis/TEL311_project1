...
figure(1)
hold on
title('Normalized Classes')
plot(X1(:, 1), X1(:, 2), 'bo');
plot(X2(:, 1), X2(:, 2), 'rs');
axis([-4.1 4.1 -4.1 4.1]); axis square;
legend('X1','X2')
hold off
...
%for i=1:3
%    tt=meas_norm(iris_labels==i-1,:);
%    [meas_reduced(i,:)] = (projectDataLDA(meas_norm(iris_labels==i-1,:), v'))
%end
...
% Get the data for each class
IRIS1_R = meas_reduced(iris_labels==0,:); 			%Samples of Class 0
IRIS2_R = meas_reduced(iris_labels==1,:); 			%Samples of Class 1
IRIS3_R = meas_reduced(iris_labels==2,:); 			%Samples of Class 2


%  Visualize the example dataset
figure(4)
hold on
plot(IRIS1_R(:, 1), IRIS1_R(:, 2), 'bo');
plot(IRIS2_R(:, 1), IRIS2_R(:, 2), 'rs');
plot(IRIS3_R(:, 1), IRIS3_R(:, 2), 'g+');
hold off
